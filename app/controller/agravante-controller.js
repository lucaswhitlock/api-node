...
exports.update = async (req, res) => {
  try {
    res.send(
      await Agravante.findByIdAndUpdate(req.params.agravanteId, req.body, {
        new: true
      })
    );
  } catch (error) {
    if (err.kind === "ObjectId" || err.name === "NotFound") {
      return res.status(404).send({
        message:
          "Não foi possível encontrar nenhum tipo de FO com o id: " +
          req.params.agravanteId
      });
    }
    return res.status(500).send({
      message:
        "Não foi possível deletar o tipo de FO com o id: " +
        req.params.agravanteId
    });
  }
};
